...
export function useBottlePhysics({
  ...
  const [params] = useState(() => ({
    driftSpeed:
      BOTTLE_PHYSICS.DRIFT_SPEED_MIN +
      Math.random() *
        (BOTTLE_PHYSICS.DRIFT_SPEED_MAX - BOTTLE_PHYSICS.DRIFT_SPEED_MIN),
    bobAmplitude:
      BOTTLE_PHYSICS.BOB_AMPLITUDE_MIN +
      Math.random() *
        (BOTTLE_PHYSICS.BOB_AMPLITUDE_MAX - BOTTLE_PHYSICS.BOB_AMPLITUDE_MIN),
    bobFrequency:
      BOTTLE_PHYSICS.BOB_FREQUENCY_MIN +
      Math.random() *
        (BOTTLE_PHYSICS.BOB_FREQUENCY_MAX - BOTTLE_PHYSICS.BOB_FREQUENCY_MIN),
  ...
  useEffect(() => {
    const updateDrift = () => {
      const speed = params.driftSpeed / 60;
    ...
    const driftInterval = setInterval(updateDrift, 1000 / 60);
  ...
  useEffect(() => {
    const updateBob = () => {
      const time = Date.now();
      const bobOffset =
        Math.sin((time / params.bobFrequency) * Math.PI * 2 + params.phase) *
        params.bobAmplitude;
    ...
    const bobInterval = setInterval(updateBob, 1000 / 60);
